...
export const MovieDetail = () => {
  ...
  return (
    <main>
      <section className='flex justify-around py-10 flex-wrap'>
        <div className='max-w-sm'>
          <img className='rounded-sm' src={image} alt="movie image" />
        </div>

        <div className='max-w-2xl text-gray-700 text-lg dark:text-white'>
          <h1 className='text-4xl font-bold text-center lg:text-left my-2'>{movie.title}</h1>
          <p className='my-4'>{movie.overview}</p>

          {movie.genres ?
            (<p className='flex mt-3 flex-wrap gap-2'>
              {movie.genres.map((genre) => {
                return (
                  <span className='text-gray-700 text-sm dark:text-gray-200 border border-gray-200 dark:border-gray-600 p-2 rounded mr-2' key={genre.id}>{genre.name}</span>
                )
              })}
            </p>)
            : ""}
